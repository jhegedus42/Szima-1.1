...
egyVonal = [
  ...
  MkFeladat "100.06" "KettőKategória átírása (KÉSZ — a 3 meztelen típus ékezetes: KettőKategóriaT (kettőCella/identitás1-2/kompozíció1-2), CPTFázis (TöltésFázis/ParitásFázis/IdőFázis/CPTTeljes), Involúció (InvolúcióAzon/InvolúcióKép) + az EsetragMorfizmus 12 konstruktorának AkH-helyesírása (NominatívuszMorf…EsszívuszMorf) + sejtCímke String→Szöveg (karakterláncbólTő IMPORT a DiracNyelv-ből — §24!) + üresSejt/kategóriaSejt/belsőSejt/külsőSejt/szenzorosÁllapot/aktívÁllapot + kettőKategóriaFő (a Fom ELÍRÁSBÓL!) + a főprogram kiírásai ékezetesen + 8 kínai szakaszcím (「一、2-范畴的定义」…「八、主程序」); ÚJ CSAPDA #24: az import … hiding NEM LÉTEZIK Idris2-ben (a parser visszaugrik és félrevezetően 'Imports must go before declarations'-t mond) — a %hide Modul.Típus direktíva a megoldás (GAN-empirikusan igazolta a saját gépemen!); + nominális típusok: két azonos alakú Kubit KÜLÖNBÖZŐ típus ('Mismatch between: Kubit and Kubit'); a strukturális megoldás (kanonikus Kubit + import public) KÜLÖN TERV-LÉPÉS / 中文：二范畴模块完成——三个裸类型带变音符、十二个格构造器按 AkH 正字法、字符串字段换成 Szöveg（按 §24 从 DiracNyelv 导入）、主程序输出全部变音符、八个中文标题；新陷阱 #24：Idris2 没有 import hiding——须用 %hide 伪指令（GAN 实证））"
    Kész Közepes "osveny_index/KettoKategoria.idr",
  MkFeladat "100.07" "CayleyDickson átírása (KÉSZ — a 5 meztelen csomagolva: (1) Valós=Double SZINONIMA → Valós REKORD (érték : Double mezővel) + teljes típusos aritmetika (valósÖsszead/Kivon/Szoroz/Negál — minden szorzás/konjugál átírva!), (2) dimencio : Nat ELÍRÁS → dimenzió : Sorszám (§24: import Alap.CsomagoltTipusok — sorTizenhat!), (3) egysegekSzama : Nat → egységekSzáma : Sorszám (16+14=30 — a dimenzió köré fűzve), (4-5) a konjugálok szabados Double-literáljai (0, (-1)) → nevezett konstansok (valósNulla/valósEgység/valósMínuszEgység/mínuszImagináriusEgység/mínuszEgységKomplex/mínuszEgységKvaternion + where-beli mínuszEgységKvaternionOktonionként); + minden azonosító ékezetes (első/második/elsőH/másodikH/elsőO/másodikO/konjugál/normaNégyzet/egység/HibajavítóKód/főJelentés — az AkH.12 szerint); + 9 kínai szakaszcím (「一、实数」…「九、主程序」); + a fv-párok párhuzamos zárójelezése a kvaternionSzoroz/oktonionSzoroz/sedenionSzoroz-ben (a backtick-es infix helyett prefix — olvashatóbb); NUMERIKUS VERIFIKÁCIÓ: Norma²(oktonionEgység)=Norma²(sedenionEgység)=1.0 — a típusos aritmetika helyessége futásig igazolva; a függő SteaneHierarchia EXIT 0 (csak a koncepciót használja, nem a neveket — az átnevezés biztonságos volt); ÚJ tanulság: a 30=SorKövetkező¹⁴ kézi láncolatban EGY zárójel hiányzott — a szem NEM látta, az awk gsub MEGSZÁMLÁLTA (13 nyitó/14 záró) — «a gép a bíra, nem a szem» / 中文：凯莱–迪克森模块完成——五个裸类型包装（实数记录化+维度/单位数 Sorszám 化+字面量入常量）、全部标识符带变音符、九个中文标题；数值验证范数平方=1.0；新教训：手工链少一个括号——眼睛没看见，awk 数出来了（机器即裁判，不是眼睛））"
    Kész Közepes "osveny_index/CayleyDickson.idr",
  MkFeladat "100.08" "DiracGammaMátrixok átírása (KÉSZ — a 3 meztelen: (1) komplexEgyenlo nyers Bool-ja → komplexEgyenlő : Igazság (ésE-vel + az ÚJ kanonikus 'EgyenlőségT Integer' instance-dzal: egyenlőE — a typeclass-szellem: instance resolution = számítás!), (2) a teszt-perem Bool-hídja → ÚJ kanonikus 'igazságÉrtéke : Igazság -> Bool' az Alap.CsomagoltTipusok-ban (az Igazság-definíció mellé — CSAK hozzáadás, MANTRA), (3) ~30 azonosító ékezetesítése AkH.12 szerint (valósRész/képzetesRész/ImagináriusEgy/NégyNégyMátrix/mező00…mező33/EgységMátrix/NullaMátrix/mátrixSzoroz/mátrixÖsszead/skalárSzorzásMátrix/sorOszlopSkalárSzorzat/GammaKettő/GammaHárom/GammaÖt/GammaNullaSzerveriHibás/BizAntikommutátor×6/BizGammaÖt*/főJelentés/DiracGammaLeírás); + a Teszt szinkron: ÚJ tesztI : String -> Igazság -> TesztEredmeny híd, a 6 dirac-tesztsor átírva; + Attekintes: DiracGammaLeírás; + a CSAPDA #24 kétszeres éles alkalmazása a Tesztben: %hide Alap.CsomagoltTipusok.Kubit ÉS %hide Alap.CsomagoltTipusok.Esetrag (a MagyarNyelvtan.Esetrag marad!) — ÚJ részletszabály: a %hide az IMPORT-LISTA VÉGÉRE kell (ha import közé ékelődik: 'Imports must go before declarations' félrevezető hiba!); a 11 Refl-bizonyítás MIND átvészelte az átírást, az integrációs Teszt 164/164 Show-teszt sikeres / 中文：狄拉克伽马矩阵完成——Bool→Igazság+新桥 igazságÉrtéke+约三十个标识符变音符化+测试同步+双重 %hide（陷阱 #24 再立功：%hide 须放导入列表末尾）；11 个 Refl 全部存活，164/164 集成测试通过）"
    Kész Közepes "osveny_index/DiracGammaMatricak.idr",
