module Muszerefal_v4

-- ═══════════════════════════════════════════════════════════════
-- MŰSZERFAL v4 — a projekt ÖSSZES kulcsmutatója egy helyen (W9)
-- INSTRUMENT PANEL v4 — ALL key indicators of the project in one place
-- 仪表盘 v4 — 项目全部关键指标集于一处（W9）
-- INSTRUMENTENTAFEL v4 — alle Schlüsselkennzahlen an einem Ort
-- לוח מחוונים גרסה 4 — כל מדדי המפתח של הפרויקט במקום אחד
-- ═══════════════════════════════════════════════════════════════
--
-- MIÉRT v4? (§13: a javítás ÚJ FÁJL — a Muszerefal_v1.idr ÉRINTETLEN.)
--   A Muszerefal_v1 SAJÁT magában nem hibás — a Hullam_3 build
--   (2026-09-05) az IMPORT-LÁNCON át bukott:
--   Muszerefal_v1 → Mondat_v1 → FazisAlgebra_v2 (törött).
--   Ez a v4 AZ EGYETLEN sort cseréli a láncon: `import Mondat_v1` →
--   `import Mondat_v2` (ami a FazisAlgebra_v3-ra van átirányítva).
--   Minden más — a 38 mezős rekord, a híd-bizonyítás, a main —
--   SZÓ SZERINT a v1 tartalma (a Mondat_v2 pontosan a v1-nek azt a
--   négy nevet exportálja, amit a v1 importált: CPTBélyegekKonst,
--   különbözőBélyegekSzáma, fázistényező, PéldaBélyegKonst).
--
...
import Mondat_v2               -- CPTBélyegekKonst, különbözőBélyegekSzáma, fázistényező, PéldaBélyegKonst (§24 —
                               -- az ÁTIRÁNYÍTOTT lánc: Mondat_v2 → FazisAlgebra_v3; a v1 → FazisAlgebra_v2 törött volt)
...
record MűszerfalMutatók where
  ...
  -- ── a nyelv (forrás: GyokSzo_v1, Fogalom_v1, Mondat_v2) ──
...
main = do
  putStrLn "════════════════════════════════════════════════════════"
  putStrLn "  SZIMA MŰSZERFAL v4 — minden kulcsmutató egy helyen (W9)"
